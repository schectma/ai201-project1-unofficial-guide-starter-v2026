...
This repository implements a searchable knowledge base. Search is powered by an AI model connected via API. Chosen corpus is `campus_life`. System answers high-level questions about course add/drop policy, dorm history, dining hall timing, course assessments, and transcript costs.
...
Chunks need to stay this size for reasonable likelihood that full answer will be present within each.

No document is large enough to be split, so leaving it at 120 is fine.
...
| 2. Every answer names a source | 5 of 5 | 4/5 | 5/5 | 5/5 | MET |
